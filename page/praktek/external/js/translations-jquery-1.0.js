/**
 * copied from: https://github.com/porduna/appcomposer/blob/master/appcomposer/static/js/translations-jquery-1.0.js
 *
 */
...
golab.i18n = function(options) {
    ...
    if (options.languageIdUrl) {
        self.languageIdUrl = options.languageIdUrl
    }
    ...
    $("meta[name='translations']").each(function(position, metaObject) {
        ...
        console.warn(`trying to get`, fullUrl)
        ...
        var currentEndpoint = self.endpoint + '/' + self.language + '/';
        if (self.languageIdUrl) {
            currentEndpoint += self.languageIdUrl
        } else {
            currentEndpoint += fullUrl
        }

        function fetchTranslations(endpoint) {
            ...
            if (self.language !== defaultLanguage && currentLanguage === defaultLanguage) {
                if (self._debug) {
                    console.warn(`trying to get`, currentEndpoint);
                }
                $.get(endpoint)
                    ...
                    .always(function () {
                        if (!$.isEmptyObject(currentRecord.translations) || fullUrl.indexOf('https://') == -1) {
                            currentRecord.pendingTranslation = false;
                            checkPending();
                        } else {
                            fullUrl = fullUrl.replace("https://", "http://");
                            currentEndpoint = self.endpoint + '/' + self.language + '/' + fullUrl;
                            fetchTranslations(currentEndpoint);
                        }
                    });
            } else {
                currentRecord.pendingTranslation = false;
